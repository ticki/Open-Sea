...
impl Font {
  pub fn new(path: &Path) -> Font {
    ...
    let glyph = font.create_character(unknown_glyph_code);
    font.cache.insert(unknown_glyph_code, glyph);
    font
  }
...
impl CharacterCache for Font {
  ...
  fn character(&mut self,
               _: graphics::types::FontSize,
               _: char ) -> &Character<Self::Texture> {
    &self.cache[&(FONT_UNKNOWN_GLYPH_CHAR as u32)]
  }
